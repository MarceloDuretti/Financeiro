...
*   **Data Management Features (CRUD):**
    ...
    *   **Transactions:** Redesigned layout prioritizing the transaction list, temporal navigation system with YoY comparison, compact KPI cards, essential filters, virtualized list for performance, and a 5-step multi-step wizard for creation/editing with progressive summaries and consistent color coding (blue for revenue, red for expense). **Three view modes:** cards (grid), list (detailed), and week (7-column calendar). **Week View Features:** Complete week visualization with buffer data for weeks crossing month boundaries, dedicated navigation (Anterior/Próxima/Hoje), and day-by-day grouping. **Drag-and-Drop:** Transactions can be dragged between days in week view using @dnd-kit/core. Visual feedback during drag (opacity/cursor changes, drop zone highlight), automatic date update (both dueDate and issueDate) via PATCH mutation, toast notifications, and real-time WebSocket updates to other clients. **KPI Consistency:** All KPIs (monetary values and counters) remain month-scoped regardless of view mode or selected week, ensuring accurate financial metrics. **Smart Data Flow:** Single query with buffer (startOfWeek to endOfWeek) for complete weeks, filtered to monthOnlyTransactions for KPIs, and further filtered by week range only in week view mode. **Transaction Codes:** Sequential integer codes (1, 2, 3...) stored in database, formatted for display as REC001/DES001 (prefix based on type + 3-digit padded code). Thread-safe generation using PostgreSQL advisory locks per (tenantId, companyId). Backfill endpoint available for assigning codes to legacy transactions. **Code Visibility:** Transaction codes displayed prominently in ALL three view modes (cards, list, week) via consistent mono-styled badges, enabling reliable voice commands like "clone REC007 por 6 meses" regardless of current view. **AI Assistant for Batch Transactions:** Advanced voice/text input with GPT-4o-mini analysis supporting: (1) Multiple specific dates ("lançamento em 02/11, 03/11, 04/11"), (2) Recurring patterns (daily/weekly/monthly/yearly), (3) Clone with modifiers - Natural language cloning with field overrides ("clone DES001 mas para Tiago Silva Ferreira" automatically fetches original transaction, applies person override, and prefills form with all original data including chart account and cost center), (4) Batch generation with 50-transaction limit, (5) Invalid date validation (e.g., 31/02 rejected), (6) Editable preview with individual transaction editing, (7) Mass editing via reference transaction. **Three-step flow:** Input → Intermediate form (AI interpretation + missing field completion + count input if needed) → Final preview (plano de contas + centro de custo assignment). **Voice Recognition Fix:** Chrome Web Speech API incorrectly transcribes "de dois mil e vinte e cinco" as "2015" instead of "2025". Frontend post-processing automatically corrects years 2015-2024 to current year (2025) ONLY in date contexts (DD/MM/YYYY format), preserving legitimate historical year references in other contexts (e.g., "lançamento de 2019" remains unchanged). **Timezone Handling:** All dates are converted to noon UTC before database storage (using Date.UTC with 12:00:00) to prevent timezone shift bugs where dates would appear as previous day due to UTC-3 conversion.
    *   **Financial Analysis:** Comprehensive analytics page featuring: (1) **DRE Hierarchical View** with multi-level account structure and real-time calculations, (2) **Yearly Evolution Charts** showing monthly trends for revenues, expenses, and net result with YoY comparison, (3) **AI-Powered Consultative Report** - Full professional financial analysis generated by GPT-4o-mini including 7 sections (Executive Summary, Revenue Analysis, Expense Analysis, Financial Indicators, Trends, Alerts & Risks, Strategic Recommendations) plus quick insight cards. **Historical Analysis:** Backend fetches 3-month comparative data (current month, previous month, same month last year) for MoM and YoY trend analysis. **PDF Export:** Complete report export using jspdf and jspdf-autotable with professional formatting including cover page, all 7 sections with color-coded headings, insights table, automatic pagination, and metadata footer. **User Experience:** Modal dialog triggered by "Análise IA" button in page header, expandable sections for organized reading, professional consultant-style writing (no emojis, descriptive text labels: ALERTA, SUCESSO, DICA, INFO). **Implementation:** Direct OpenAI API calls (cost-optimized GPT-4o-mini), context-aware analysis including company name and business metrics, filename format: `Relatorio_Consultivo_MES_ANO.pdf`.
