...
*   **Data Management Features (CRUD):**
    ...
    *   **Chart of Accounts:** Hierarchical tree structure (max 5 levels) with auto-seeded defaults, VS Code-inspired UI, and auto-generated codes using materialized path. **AI-Powered Generation:** Intelligent chart of accounts generation using OpenAI GPT-4o-mini with contextual analysis. AI performs two-layer analysis: (1) detects specific items mentioned by user (e.g., "água", "luz", "AWS") and guarantees their inclusion, (2) analyzes business type/industry to generate relevant additional accounts. Generates 50-70 detailed analytical accounts ready for real-world use. **Editable Preview:** Before confirming creation, users can add new subaccounts (with auto-code generation and max 5-level validation) or delete unwanted subaccounts (with validation preventing deletion of parent accounts with children). **Root Account Protection:** The 5 root accounts (1-Ativo, 2-Passivo, 3-Patrimônio Líquido, 4-Receitas, 5-Despesas) are protected and cannot be modified or deleted. Visual indicators (lock icon + "Protegida" label) clearly mark protected accounts, and delete buttons are hidden for root accounts. Full CRUD in preview mode ensures complete customization of subaccounts before database insertion.
